...
On an NVIDIA Jetson, choose the **NVIDIA Jetson** tab in each section: it
installs the application and the full pose stack together.
...
(jetson-install)=
These steps install the application and the full pose stack on a Jetson in a
single environment: SLEAP-NN, DeepLabCut-Live, CUDA PyTorch, ONNX Runtime and
TensorRT. They also build FFmpeg with the Jetson's hardware H.264 encoder for
recording. For a rig without pose tracking, steps 1, 2, 6, 8 and 10 are enough.

Every step was run and verified on the machine listed in
[Jetson tested configuration](#jetson-tested-configuration). Several steps look
unusual. Each exists because the obvious version failed, and the reason is given
with the step. Skip one and the environment usually still installs, then runs
inference on the CPU with no error to say why.

:::{rubric} What this procedure assumes
:::
...
0.000 px (see {ref}`Verify <jetson-verify>`).
:::

(jetson-system-packages)=
:::{rubric} System packages
:::
...
:::{rubric} Put the environment on the NVMe
:::
...
:::{rubric} 1. Create the environment and get the code
:::

```bash
python3.10 -m venv "$VENV"
"$VENV/bin/pip" install --upgrade pip setuptools wheel
git clone https://github.com/pyBehaviorLab/code ~/Documents/pyBehaviorLab
```

**Do not pass `--system-site-packages`.** The system Python carries NumPy 1.21
and an OpenCV 4.8 build that would shadow the versions installed below.
TensorRT, the one system package that is needed, is linked in explicitly in
step 4.

:::{rubric} 2. Pin the versions
:::
...
(jetson-torch)=
:::{rubric} 3. Install CUDA PyTorch and ONNX Runtime
:::
...
(jetson-tensorrt)=
:::{rubric} 4. Link the JetPack TensorRT into the environment
:::
...
:::{rubric} 6. Install sleap-nn's dependencies, DeepLabCut-Live and the application
:::
...
:::{rubric} 7. Keep exactly one OpenCV
:::
...
:::{rubric} 8. Build the Cython extensions
:::
...
:::{rubric} 9. Record the environment
:::
...
[Record what you installed](#record-what-you-installed).

(jetson-ffmpeg)=
:::{rubric} 10. Build FFmpeg with the hardware encoder
:::
...
- **A very noisy frame** can make the encoder fail and stop encoding for the rest
  ...
  size ([Jetson tested configuration](#jetson-tested-configuration)).
::::
...
On a Jetson, activate the environment and start from the clone:
...
it and falls back to CPU otherwise; on a Jetson, see
{ref}`step 10 <jetson-ffmpeg>`. A C compiler is optional: the Cython
...
2. **Connect the board** over USB. It appears as a serial port. On Linux and
   Jetson, install the {ref}`udev rule <jetson-system-packages>` so the port is
   accessible.
...
The pose stack is already installed by steps 3 to 7 of the
{ref}`Jetson install <jetson-install>`. What remains is preparing the models.
Skip this if the rig only uses the exported ONNX files. Everything else runs as
shipped.

(jetson-models)=
:::{rubric} TensorRT engines are built on the Jetson
:::
...
built for a different box count or precision is not used. Keep the pose batch
size small.
...
ONNX file ([Jetson tested configuration](#jetson-tested-configuration)).
...
(jetson-dlc-snapshots)=
:::{rubric} DeepLabCut PyTorch snapshots saved on Windows
:::
...
:::{rubric} TensorFlow models
:::

DeepLabCut 2.x models stored as TensorFlow snapshots (`pose_cfg.yaml` plus
`.pb`) need TensorFlow, which the Jetson environment does not include. Export
them to ONNX on a workstation.
::::
...
machine" on Linux; check 3 covers inference instead.

:::{rubric} 2. SLEAP against the reference keypoints
:::
...
folder given to `--model` (see {ref}`preparing the models <jetson-models>`).
...
:::{rubric} 3. Every model through every runtime
:::
...
- The two `test_dlc_engine` checkpoint tests hit the Windows path described in
  {ref}`DeepLabCut PyTorch snapshots <jetson-dlc-snapshots>`.

A failure outside these files is worth investigating.

:::{rubric} 5. The application
:::
...
:::{rubric} 6. Hardware encoding and TensorRT in a session
:::
...
- `nvmpi=False`, or `Using libx264` for every box: the application is not using
  the FFmpeg from {ref}`step 10 <jetson-ffmpeg>`.
- `onnxruntime` in the last line: the cache has no engine for this model, box
  count and precision. See {ref}`preparing the models <jetson-models>`.
::::
...
| Recording falls back to CPU encoding | FFmpeg on `PATH` was built without the hardware encoder | Install a full FFmpeg build |
::::
...
| TensorRT or `auto` selected, the log shows `onnxruntime` | No engine in the cache for this model, box count and fp16 setting | Export one; see the Jetson tab of [Add pose estimation](#add-pose-estimation) |
| Log says `Jetson detected but h264_v4l2m2m not in this FFmpeg build`, every box records with libx264 | The system FFmpeg is on `PATH`; its `h264_v4l2m2m` cannot reach the Orin's encoder | Step 10 of the Jetson install, then start the application from the activated environment |
...
| Serial ports `Permission denied`, or a board only connects after `sudo chmod 666 /dev/ttyACM*` | No udev rule, so the ports are `root`/`dialout` only again after every replug or reboot | Install the pyboard udev rule once (System packages in the Jetson install) |
...
its own environment. The Jetson is the exception: it has to use 3.10, and
installs sleap-nn with `--ignore-requires-python`.
