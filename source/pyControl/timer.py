...
def pause_state_timers():
    # Move active STATE_TYP timers (timed_goto_state) to paused_timers, with remaining time.
    global active_timers, paused_timers
    paused_timers += [
        fw.Datatuple(t.time - fw.current_time, *t[1:])
        for t in active_timers
        if t.type == fw.STATE_TYP
    ]
    active_timers = [t for t in active_timers if t.type != fw.STATE_TYP]


def unpause_state_timers():
    # Move paused STATE_TYP timers back to active, with absolute time.
    global active_timers, paused_timers
    active_timers += [
        fw.Datatuple(t.time + fw.current_time, *t[1:])
    ...
    paused_timers = [t for t in paused_timers if t.type != fw.STATE_TYP]
    active_timers.sort(reverse=True)


def peek_next_state_target():
    # Return the state ID of the soonest-firing STATE_TYP timer, or None.
    # active_timers is sorted reverse=True, so the soonest firing is at
    # the END (largest list index, smallest .time value).
    for t in reversed(active_timers):
        if t.type == fw.STATE_TYP:
            return t.content
    return None
