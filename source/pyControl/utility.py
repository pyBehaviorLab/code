...
# Coordinates class (from maze) -----------------------------------------------
# Separate namespace for task coordinates (tracking data pushed in from GUI)
# via the `b'c'` serial command handled in framework.receive_data.
...
# Stage / nav shims -----------------------------------------------------------
# Names exposed for `from pyControl.utility import *` task-file calls.
# pause/resume/next/prev work on every task; stage_index/remaining/count are
# meaningful only when the task defines `stage_durations`. Implementation
# lives in state_machine.
...
def stage_index():
    from . import state_machine as sm
    return sm.stage_index()


def stage_remaining():
    from . import state_machine as sm
    return sm.stage_remaining()


def stage_count():
    from . import state_machine as sm
    return sm.stage_count()
